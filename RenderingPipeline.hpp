...
template <typename VertexInput, typename VertexOutput, typename Uniform> struct Pipeline {
	...
	static void resolveScanline(const FragmentShader& frag, const Uniform& uniform, int scanline, int minX, int maxX, float rArea,
	...
	static void fillBottomFlatTriangle(const FragmentShader& frag, const Uniform& uniform, float rArea, const VertexOutput& v0, const VertexOutput& v1, const VertexOutput& v2) {
		...
		const int minY = int(std::trunc(v0.COORDS.y));
		const int minX = int(std::trunc(v2.COORDS.y));
		for(int i = minY; i < minX;++i) {
			...
			resolveScanline(frag,uniform,i,int(std::trunc(curx1)),int(std::trunc(curx2)),rArea,v0,v1,v2);
		}
	}
	static void fillTopFlatTriangle(const FragmentShader& frag, const Uniform& uniform, float rArea, const VertexOutput& v0, const VertexOutput& v1, const VertexOutput& v2) {
	...
	static void renderVertexOutput(const FragmentShader& frag, const Uniform& uniform, const VertexOutput& v0, const VertexOutput& v1, const VertexOutput& v2) {
		...
		if (m->COORDS.y == t->COORDS.y)
		{
			...
			fillTopFlatTriangle(frag, uniform, rArea, *l, *r, *b);
		}
		else if (m->COORDS.y == b->COORDS.y)
		{
		const VertexOutput *l = m, *r = b;
			if (l->COORDS.x > r->COORDS.x) std::swap(l, r);
			fillBottomFlatTriangle(frag, uniform, rArea, *t, *l, *r);
		}
		else
		  {
			// Split the triangle
	...
	static void renderTriangle(const VertexShader& vert, const FragmentShader& frag, const Uniform& uniform, const VertexInput& v0, const VertexInput& v1, const VertexInput& v2) {
		const VertexOutput o0 = vert(uniform,v0);
		const VertexOutput o1 = vert(uniform,v1);
		const VertexOutput o2 = vert(uniform,v2);
		renderVertexOutput(frag,uniform,o0,o1,o2);
	}
	static void renderTriangles(const VertexShader& vert, const FragmentShader& frag, const Uniform& uniform, const VertexInput* vertices, size_t vertexCount) {
		for(size_t i = 0; i < vertexCount; i += 3) {
			renderTriangle(vert,frag,uniform,vertices[i],vertices[i+1],vertices[i+2]);
		}
	}
	static void renderTriangles(const VertexShader& vert, const FragmentShader& frag, const Uniform& uniform, const VertexInput* vertices, unsigned* indices, size_t indexCount) {
		for(size_t i = 0; i < indexCount; i += 3) {
			renderTriangle(vert,frag,uniform,vertices[indices[i]],vertices[indices[i+1]],vertices[indices[i+2]]);
		}
	}
};
